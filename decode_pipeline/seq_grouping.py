...
def grouping(in_basecalled_dir, in_pickle_path, bcd_path, out_grouping_res_dir):
    ...
    filelist = os.listdir(in_basecalled_dir)
    fastq_list = list(
    ...
    for fastq in fastq_list:
        all_seqs += [(str(item.id), str(item.seq)) for item in SeqIO.parse(os.path.join(in_basecalled_dir, fastq), 'fastq')]
    # all_seqs = [(str(item.id), str(item.seq)) for item in SeqIO.parse('/data/biolab-nvme-pool1/zhaoxy/comhedges/sustech_fqy_bonito_test/BONITO_dna_r9.4.1_e8_sup@v3.3_20230608_1759_MN26597_FAS92658_95f4ad6b.fastq', 'fastq')]
    print('All seqs read...')
    
    with tqdm.trange(len(all_seqs), desc='genome grouping...', mininterval=3.0) as tbar:
        for seq in all_seqs:
            try:
    ...
    if not os.path.exists(out_grouping_res_dir):
        os.makedirs(out_grouping_res_dir)

    for idx in tqdm.tqdm(range(ref_num)):
        with open(out_grouping_res_dir+f'grouping_res_{idx}.fasta', 'w') as f:
...
if __name__ == "__main__":
    in_basecalled_dir = r'/data/biolab-nvme-pool1/lijy/sim_readuntil_fastq/CNNLSTMselnet_sustag384_t0.20finetuned_0_5m/'

    in_pickle_path = r'./CNNLSTMselnet_sustag384_t0.20finetuned_0_5m.P'

    bcd_path = r'/home/lijy/workspace/fasta/barcodeljy_384.fasta'
    
    out_grouping_res_dir = r'./grouping_res_CNNLSTMselnet_sustag384_t0.20finetuned_0_5m/'
    
    grouping(in_basecalled_dir, in_pickle_path, bcd_path, out_grouping_res_dir)
