...
from sustag.model.model_selec_CNNLSTM import CNN_LSTM, SelectiveNet, SelectiveLoss
...
def train(num_class: int, hidden_dim: int, batch_size: int, epochs: int, learning_rate: float,
    ...
    features = CNN_LSTM(num_class, dropout_rate, hidden_dim).to(current_device)
    
    model = SelectiveNet(features, 79 * hidden_dim, num_class, dropout_rate=dropout_rate).to(current_device)    
    model_summary(model)
    ...
    model_out_path = f"./checkpoints/CNNLSTMselnet/CNNLSTMselnet_sustag96_e{epochs}_b{batch_size}_t{total_rate:>.2f}_c{coverage:>.2f}_4kun.pth"
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_i, (X, Y) in enumerate(train_loader):
            ...
            # print('out_aux:', out_aux)
            
            loss_dict = OrderedDict()
            # loss dict includes, 'empirical_risk' / 'emprical_coverage' / 'penulty'
            # selective_loss, loss_dict = SelectiveCELoss(out_class, out_select, Y)
            ...
            if batch_i % int((size//batch_size)*0.4) == 0:
                ...
                # selacc = selective_acc(out_class,  out_select, Y, threshold=0.5)
                # cov = selective_coverage(out_select, threshold=0.5)
                # print(f"selacc: {selacc} coverage: {cov}")
