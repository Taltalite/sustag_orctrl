...
class ReadUntilClient(object):
    ...
    def __init__(
        ...
        self.logger.debug(
            f"ReadUntilClient __init__ received: "
            f"first_channel={first_channel}, last_channel={last_channel}, "
            f"cache_type={cache_type.__name__}"
        )
        ...
        # MODIFICATION: Use a set to track actioned read_ids
        self.actioned_read_ids = set()
        ...
        # self.logger.debug(f"self.last_channel: {self.last_channel}")
        ...
        device_channel_count = self.connection.device.get_flow_cell_info().channel_count
        
        # Only use device channel count if the user did NOT provide a value
        if self.first_channel is None:
            self.first_channel = 1
            self.logger.debug(f"first_channel not set by user, defaulting to 1.")
            
        # self.logger.debug(f"self.last_channel: {self.last_channel}")
        
        if self.last_channel is None:
            self.last_channel = device_channel_count
            self.logger.debug(f"last_channel not set by user, defaulting to device count: {device_channel_count}.")
        
        if self.CacheType.__name__ == "PreallocAccumulatingCache":
            self.cache_size = self.last_channel - self.first_channel + 1
        else:
            # For other caches, size can be independent of channel count, but we'll default it
        ...
        if self.CacheType.__name__ == "PreallocAccumulatingCache":
            ...
            # )
            # self.cache_size = self.last_channel
            self.cache_size = (self.connection.device.get_flow_cell_info().channel_count)
    ...
    def run(self, **kwargs):
        ...
        """
        self.logger.debug(f'run kwargs: {kwargs}')
    ...
    def get_read_chunks(self, batch_size=1, last=True, min_chunk_length=0):
        ...
        filtered_data = []
        for channel, read_item in data:
            # MODIFICATION: The primary filter is now based on read_id
            if read_item.id in self.actioned_read_ids:
                continue

            if read_item.chunk_length < min_chunk_length:
                continue

            filtered_data.append((channel, read_item))
    ...
    def unblock_read(self, read_channel, read_id, duration=0.1):
        ...
        self.actioned_read_ids.add(read_id)
        self._send_action(read_channel, read_id, "unblock", duration=duration)
    ...
    def stop_receiving_read(self, read_channel, read_id):
        """Request to receive no more data for a read using its read_id."""
        self.actioned_read_ids.add(read_id)
        self._send_action(read_channel, read_id, "stop_further_data")
    
    def _send_action(self, read_channel, read_id, action, **params):
        """Generate and send an action request using read_id."""
        action_id = str(uuid.uuid4())
        # MODIFICATION: The action payload now uses 'id' instead of 'number'
        action_kwargs = {
            "action_id": action_id,
        ...
        if action == "stop_further_data":
            ...
            action_kwargs[action] = data_pb2.GetLiveReadsRequest.UnblockAction(**params)
        else:
            raise ValueError("Action must be 'stop_further_data' or 'unblock'.")
    ...
    def _run(self, **kwargs):
        ...
        self.logger.debug(f'_runner(**kwargs): {kwargs}')
    ...
    def _process_reads(self, reads):
        ...
        for reads_chunk in reads:
            ...
            for read_channel, read_data_object in reads_chunk.channels.items():
                # Just put the object in the queue. It already has the .id
                self.data_queue[read_channel] = read_data_object
            
            now = time.time()
            log_interval_secs = 60 * 15  # 15 mins
            if last_msg_time + log_interval_secs < now:
                avg_samples_behind = 0

                if read_count > 0:
                    avg_samples_behind = samples_behind / read_count
                self.logger.info(
