    ...
    def __setitem__(self, key, value):
        ...
        with self.lock:
            if key not in self._dict:
                # Key not in _dict
                self._dict[key] = value
                self.missed += 1 # A new read for this channel
            else:
                # Key exists, compare by read_id
                if self._dict[key].id == value.id:
                    # Same read, accumulate raw_data
                    if len(self._dict[key].raw_data) < self._max_raw_signal:
                        ...
                        if hasattr(self._dict[key], 'chunk_classifications') and hasattr(value, 'chunk_classifications'):
                            self._dict[key].chunk_classifications.extend(value.chunk_classifications)
                    self.replaced += 1
                else:
                    # New read on the same channel, replace it
                    self._dict[key] = value
                    self.missed += 1
